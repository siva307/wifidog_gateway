...
openssl_verify_peer_cb(int ok, X509_STORE_CTX *ctx) {
  ...
  if (err != X509_V_OK) {
    CP_ERROR(1, "peer certificate error: #%d : %s\n", 
...
openssl_use_certificate(openssl_env *env, char *file) {
  if (file)
    if (SSL_CTX_use_certificate_file(env->ctx, file, SSL_FILETYPE_PEM) > 0)
      return 1;
  CP_ERROR(1, "could not load certificate file %s\n",file);
  return 0;
}
...
openssl_use_privatekey(openssl_env *env, char *file) 
{
  ...
  if (file) {
    ...
    {
            CP_ERROR(1,"private keyfile success");
            if((err2 = SSL_CTX_check_private_key(env->ctx))> 0)
            {
                    CP_ERROR(1,"check private key file success");
                    return 1;
            }
            else{
                CP_ERROR(1,"check private key file failed");
                return 0;
            }
        ...
        CP_ERROR(1,"private key file failed ");
        return 0;
  }
#if 0
    {
...
openssl_cacert_location(openssl_env *env, char *file, char *dir) {
  ...
  if (!err)
    CP_ERROR(1, "unable to load CA certificates.\n");
  return err;
}
...
openssl_tmp_genkeys(openssl_env *env) {
  ...
  if ((env->tmpKeys[OPENSSL_TMPKEY_RSA512] = RSA_generate_key(512, RSA_F4, NULL, NULL)) == NULL) {
    CP_ERROR(1, "could not generate tmp 512bit RSA key\n");
  }

  if ((env->tmpKeys[OPENSSL_TMPKEY_RSA1024] = RSA_generate_key(1024, RSA_F4, NULL, NULL)) == NULL) {
    CP_ERROR(1, "could not generate tmp 1024bit RSA key\n");
  }

  if ((env->tmpKeys[OPENSSL_TMPKEY_DH512] = openssl_dh_tmpkey(512)) == NULL) {
    CP_ERROR(1, "could not generate tmp 512bit DH key\n");
  }
...
#endif
    if (!openssl_use_certificate(env, CP_SSL_CERT_FILE) ||
	!openssl_use_privatekey(env, CP_SSL_KEY_FILE)) {
      CP_ERROR(1, "failed reading setup sslcertfile and/or sslkeyfile");
      return 0;
    }
    else{
        CP_ERROR(1,"sslcertifile and sslkey file reading success");
    }

   // if (_options.sslcafile)
     {
      if (!openssl_cacert_location(env, CP_SSL_CACERT_FILE, 0)) {
	CP_ERROR(1, "failed reading sslcafile");
	return 0;
      }
    }
    CP_ERROR(1,"Lets go for SSL ready");
    env->ready = 1;
    return err;
  }
#else
  CP_ERROR(1,"MatrixSSL Setup:");
  CP_ERROR(1,"SSL cert: %s",_options.sslcertfile);
  CP_ERROR(1,"SSL key: %s",_options.sslkeyfile);
  CP_ERROR(1,"SSL pass: %s",_options.sslkeypass?_options.sslkeypass:"null");
  CP_ERROR(1,"SSL ca: %s",_options.sslcafile?_options.sslcafile:"null");
  if ( matrixSslReadKeys( &env->keys,
			  _options.sslcertfile, 
			  _options.sslkeyfile, 
			  _options.sslkeypass, 
			  _options.sslcafile ) < 0 ) {
    CP_ERROR(1, "could not load ssl certificate or and/or key file");
    return 0;
  }
...
static int certValidator(sslCertInfo_t *t, void *arg) {
  CP_ERROR(1,"MatrixSSL: certValidator()");
  return 1;
}
#endif
...
openssl_connect_fd(openssl_env *env, int fd, int timeout) {
  ...
  if (SSL_connect(c->con) < 0) {
    ...
    while ((error = ERR_get_error())) {
      CP_ERROR(1,"TLS: %s", ERR_error_string(error, NULL));
      is_error = 1;
    }
#endif
    if (is_error) {
...
CP_ERROR(1,"I am in function called openssl check accept");
#ifdef HAVE_OPENSSL  
  int r;
  ...
  if (!SSL_is_init_finished(c->con))
  {
    ...
    if ((r = SSL_accept(c->con)) <= 0) {
         printf("SSL_accept() returned %d\n", r);
    int err = SSL_get_error(c->con, r);
	  char error[65535];
      ...
      CP_ERROR(1,"SSL error want read and write");
	return 1;
	
      case SSL_ERROR_SYSCALL:
	if (errno != EINTR) {
	  if (errno > 0)
	    CP_ERROR(1, "SSL handshake interrupted by system [Hint: Stop button pressed in browser?!]");
	  else
	    CP_ERROR(1, "Spurious SSL handshake interrupt [Hint: Usually just one of those OpenSSL confusions!?]");
	}
	break;
      }
      return -1;
	...
	if (SSL_get_verify_result(c->con) != X509_V_OK) {
	  CP_ERROR(1,"auth_failed: %s", subj);
	  X509_free(peer_cert);
	  return -1;
	}
		
	CP_ERROR(1,"auth_success: %s", subj);
...
openssl_accept_fd(openssl_env *env, int fd, int timeout, struct redir_conn_t *conn) {
  ...
  if (!c) return NULL;
    ...
    if (!env || !env->ready) {
    CP_ERROR(1, "SSL not available!");
     return NULL;
   }
    CP_ERROR(1,"Opensssl accept fd");
  c->env = env;
#ifdef HAVE_OPENSSL
  c->con = (SSL *)SSL_new(env->ctx); 
  CP_ERROR(1,"SSL new created");
#elif  HAVE_MATRIXSSL
...
openssl_error(openssl_con *con, int ret, char *func) {
  ...
  return err;
#else
  CP_ERROR(1, "ssl error in %s", func);
  return 0;
#endif
}
...
openssl_read(openssl_con *con, char *b, int l, int t)
{
  ...
  int fd = con->sock;
  int retries = 0;
  CP_INFO(1, "%s(): reading from fd=%d", __func__, fd);
 ...
 repeat_read:
  ...
  if (t && !(openssl_pending(con))) {
    ...
    if (SSL_get_rfd(con->con) < 0) {
      CP_ERROR (1,"%s(): BIO is not correct\n", __func__);
      return -1;
    }
    FD_SET(fd, &rfds);
    FD_SET(fd, &wfds);
...
openssl_write(openssl_con *con, char *b, int l, int t)
{
  ...
  CP_INFO (1, "%s() sent=%d", __func__, sent);
  return sent;
}

int cp_send(void *conn,char *buffer,int size,int do_ssl)
{
  if(do_ssl) {
    return openssl_write((openssl_con *)conn,buffer,size,0);
  } else {
    return send((int)conn,buffer,size,0);
  }
}
...
#if(_debug_)
  CP_ERROR(1,"Freeing SSL environemnt");
#endif
#ifdef HAVE_OPENSSL
...
openssl_pending(openssl_con *con) {
  if (con->con) {
    ...
    /*CP_ERROR(1,"openssl_pending(%d)", pending);*/
    return pending;
  }
  return 0;
}
